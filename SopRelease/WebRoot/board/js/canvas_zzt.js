(function(){
    var data = {
        ...
        starRate:[1350,1500,1450,1370,1480,1270,1390],
        starNum:[0,3,8,12,15,16,20],
        rectColor:["#57B0FF","#57B0FF","#57B0FF","#57B0FF","#57B0FF","#57B0FF","#57B0FF"],
    };
    var canvas,ctx;
    ...
    var init = function(data){
        canvas = document.getElementById("canvas_zhuzt");
    ...
    var drawXAxis = function(xAxis){
        ...
        for(var i=0; i<len; i++){
            //画标签，默认字体为12个像素
            ctx.font = "normal normal bold 45px 微软雅黑";
            ctx.fillStyle = "#000000";
            //字体居中
            ctx.fillText(xAxis[i],x_scale+(i+0.5)*stepWidth-xAxis[i].length*14/2,canvas.height-y_scale + 45);
            stepXArr.push(x_scale+(i+1)*stepWidth+60);   //调整x轴上的字体与柱状图对齐
        }
        ctx.stroke();
        //加箭头
    ...
    var drawYaxis = function(maxValue,step){
        ...
        for(var i=1; i<=step; i++){
            ...
            //字体居中
            ctx.fillText(maxValue/step*i,x_scale-120,canvas.height-y_scale-stepHeight*i);
            stepYArr.push(canvas.height-y_scale-stepHeight*i-20);  //调整数字与y轴高度
        }
        ctx.stroke();
        //加箭头
        drawArrow(x_scale,y_scale,true);
        //加Y轴顶部字体
        ctx.fillText(str1,x_scale-120,y_scale-30);
        ctx.fillText(str2,x_scale-120,y_scale+13);
    }
    ...
    var drawRect = function(starRate,colorArr){
        ...
        for(var i=0,len=starRate.length;i<len;i++){
            ...
            ctx.fillRect(stepXArr[i]-stepWidth/2-rectWidth/2-20,canvas.height-y_scale-starRate[i]*heightVal,rectWidth,starRate[i]*heightVal);
            ctx.fill();
        }
    }
    ...
    var mouseMove = function(e){
        ...
        for(var i=0;i<data.starRate.length;i++){
            if(x>stepXArr[i]-stepWidth*3/4&&x<stepXArr[i]-stepWidth/4&&y<canvas.height-y_scale&&y>canvas.height-y_scale-data.starRate[i]*heightVal&&data.starRate[i]!=0){
                ...
                //显示数字
                $("#show_num").html('<div class="show_num">'+data.starRate[i]+'</div>');
                $("#show_num").css({
                    "left":stepXArr[i]-stepWidth*3/4+10,
                    "top":canvas.height-y_scale-data.starRate[i]*heightVal
                });
                break;
            }else{
                //重绘矩形
