...
public class WebSocketMessageInboundPool {
	...
	//向某几个用户发送消息
	public static void sendMessageToUsers(String[] userlist,String message){
		try {
			//向多个指定的用户发送数据
			System.out.println("向多个指定的工位发送信息");
			System.out.println("userlist : " + userlist.toString() + " ,message : " + message);
			//循环查询
			for(int i=0;i<userlist.length;i++){
				WebSocketMessageInbound inbound = connections.get(userlist[i]);
				if(inbound != null){
					inbound.getWsOutbound().writeTextMessage(CharBuffer.wrap(message));
				}
			}
